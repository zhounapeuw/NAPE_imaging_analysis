...
def load_rois_for_session(session_folder):
    ...
    for file in os.listdir(root_dir):
        if file.endswith(".sima"):
            sima_folder = os.path.join(root_dir, file)
    ...
    # roi_polygons is a list with len of number of ROIs. Each entry is a shapely Polygon made from the ROI mask vertices
...
def calculate_neuropil_signals(h5filepath, neuropil_radius, min_neuropil_radius,
    ...
    for frame in range(numframes):
        ...
        print('temp {} spatialweights {}'.format(str(temp.shape),str(spatialweights.shape)))
...
# tmp
roi_centroids, im_shape, roi_polygons = calculate_roi_centroids(root_dir)
roi_masks = calculate_roi_masks(roi_polygons, im_shape)

simadir = os.path.splitext(h5filename)[0]+'_mc.sima'

dataset = sima.ImagingDataset.load(os.path.join(root_dir, simadir))

mean_roi_response = np.nansum(roi_masks*dataset.time_averages[:,:,:,0], axis=(1,2))/np.sum(roi_masks, axis=(1,2))
    
# sima divides ROI time-series by the mean response; reverse this
npyfile = os.path.splitext(h5filename)[0]+'_sima_extractedsignals.npy'
signals = np.squeeze(np.load(os.path.join(root_dir, npyfile)))
signals *= mean_roi_response[:,None]
...
if 'h5' in locals():
    h5.close() # sometime error arises if prior h5 file opening isn't properly closed
