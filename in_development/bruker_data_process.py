...
filename = 'vj_ofc_imageactivate_001_20200813-014'
root_session_folder = r'D:\bruker_data\Charles\vj_ofc_imageactivate_001_20200813\vj_ofc_imageactivate_001_20200813-014'

flag_threshold_LUT = False # set to true to threshold LUT's lower bound value to `LUT_lower_thresh`
LUT_lower_thresh = 2 # in percentage of maximum value across the image(s)
...
save_fname = os.path.join(root_session_folder, filename)
...
### start chunking and loading data
chunk_raw_mean = np.empty([n_chunks, frame_shape[0], frame_shape[1]])
chunk_processed_mean = np.empty([n_chunks, frame_shape[0], frame_shape[1]])
...
for idx, chunk_frames in enumerate(chunked_frame_idx):
    ...
    else: # in case you want to troubleshoot and visualize the data
        
        scaled_raw_chunk = deepcopy(loaded_tiffs) # to make a true copy instead of a python reference; since we will perform actions on this copy
        chunk_raw_mean[idx,...] = np.mean(scaled_raw_chunk, axis=0)
        
        processed_chunk = threshold_img(loaded_tiffs, thresh_percent=1)
        chunk_processed_mean[idx,...] = np.mean(processed_chunk, axis=0) 
        if start_idx < displayed_slice < end_idx:
            displayed_raw_slice = loaded_tiffs[displayed_slice,...]
            displayed_processed_slice = scaled_raw_chunk[displayed_slice,...]
...
#fdir = r'D:\bruker_data\Adam\20x\aac_d01_replete\test_sync-016'
#fname = r'test_sync-016'
...
def bruker_concatenate_analog(fname, fpath):
    ...
    for cycle_idx, cycle_path_csv in enumerate(glob_analog_csv):
        ...
        if cycle_idx == 0: # initialize pd dataframe with first cycle's data
            ...
            cycle_df['cumulative_time'] = cycle_df['Time(ms)'].values + last_cumulative_time + 1 # add 1 so that new cycle's first sample isn't the same as the last cycle's last sample
    ...
    for analog_column_name in analog_column_names:
        analog_concat[analog_column_name + '_diff'] = np.append(np.diff(analog_concat[analog_column_name]) > 4, False) # add a false to match existing df length
